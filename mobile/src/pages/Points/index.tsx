...
const Points = () => {
    ...
    useEffect(() => {
      async function loadPosition() {
        ...
        if(status !== "granted") {
          Alert.alert("Ooops...", "Precisamos da sua permissão para poder procurar os pontos de coleta!")
          return;
        }
    ...
    function handleChangeItemsToCollect(id: number) {
      ...
      if(!itemIsInsertedInArray) {
        ...
        const newItemsCollected = selectedItems.filter(item => item !== id)
            ...
            >
              {items.map(item => (
                <TouchableOpacity 
                  ...
                  activeOpacity={0.6}
                  onPress={() => handleChangeItemsToCollect(item.id)}
                >
